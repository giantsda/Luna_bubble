...
template<int dim>
  ...
  public:
    ...
    LevelSetSolver (const unsigned int degree_LS, const unsigned int degree_U,
		    const double time_step, const double cK, const double cE,
		    const bool verbose, std::string ALGORITHM,
  ...
  private:
    ...
    compute_solution (PETScWrappers::MPI::Vector &unp1,
		      PETScWrappers::MPI::Vector &un, std::string algorithm);
    void
    ...
    PETScWrappers::MPI::SparseMatrix Cx_matrix, CTx_matrix, Cy_matrix,
	CTy_matrix, Cz_matrix, CTz_matrix;
    PETScWrappers::MPI::SparseMatrix dLij_matrix;
    // FOR ENTROPY VISCOSITY
    PETScWrappers::MPI::SparseMatrix EntRes_matrix, SuppSize_matrix,
	dCij_matrix;
    ...
    PETScWrappers::MPI::Vector MPP_uL_solution, NMPP_uH_solution,
	MPP_uH_solution;
...
template<int dim>
  LevelSetSolver<dim>::LevelSetSolver (
      ...
      parallel::distributed::Triangulation<dim> &triangulation,
      MPI_Comm &mpi_communicator) :
      mpi_communicator (mpi_communicator), degree_LS (degree_LS), dof_handler_LS (
	  triangulation), fe_LS (degree_LS), degree_U (degree_U), dof_handler_U (
	  triangulation), fe_U (degree_U), time_step (time_step), cE (cE), cK (
	  cK), verbose (verbose), ALGORITHM (ALGORITHM), TIME_INTEGRATION (
	  TIME_INTEGRATION), pcout (
...
template<int dim>
  ...
  LevelSetSolver<dim>::setup ()
  {
    ...
    /////////////////////////////////////////////////////////
    uStage1.reinit (locally_owned_dofs_LS, locally_relevant_dofs_LS,
		    mpi_communicator);
    uStage2.reinit (locally_owned_dofs_LS, locally_relevant_dofs_LS,
		    mpi_communicator);
    unm1.reinit (locally_owned_dofs_LS, locally_relevant_dofs_LS,
		 mpi_communicator);
    un.reinit (locally_owned_dofs_LS, locally_relevant_dofs_LS,
	       mpi_communicator);
    MPP_uL_solution_ghosted.reinit (locally_owned_dofs_LS,
				    locally_relevant_dofs_LS, mpi_communicator);
    ...
    locally_relevant_solution_vz_old.reinit (locally_owned_dofs_U,
					     ...
					     mpi_communicator);
    // LIMITERS (FCT)
    R_pos_vector.reinit (locally_owned_dofs_LS, locally_relevant_dofs_LS,
			 mpi_communicator);
    R_neg_vector.reinit (locally_owned_dofs_LS, locally_relevant_dofs_LS,
			 mpi_communicator);
    ////////////////////
    // SETUP MATRICES //
    ////////////////////
	...
	dsp, dof_handler_LS.n_locally_owned_dofs_per_processor (),
	mpi_communicator, locally_relevant_dofs_LS);
    MC_matrix.reinit (mpi_communicator, dsp,
		      dof_handler_LS.n_locally_owned_dofs_per_processor (),
		      dof_handler_LS.n_locally_owned_dofs_per_processor (),
		      Utilities::MPI::this_mpi_process (mpi_communicator));
    Cx_matrix.reinit (mpi_communicator, dsp,
		      dof_handler_LS.n_locally_owned_dofs_per_processor (),
		      dof_handler_LS.n_locally_owned_dofs_per_processor (),
		      Utilities::MPI::this_mpi_process (mpi_communicator));
    CTx_matrix.reinit (mpi_communicator, dsp,
		       dof_handler_LS.n_locally_owned_dofs_per_processor (),
		       dof_handler_LS.n_locally_owned_dofs_per_processor (),
		       Utilities::MPI::this_mpi_process (mpi_communicator));
    Cy_matrix.reinit (mpi_communicator, dsp,
		      dof_handler_LS.n_locally_owned_dofs_per_processor (),
		      dof_handler_LS.n_locally_owned_dofs_per_processor (),
		      Utilities::MPI::this_mpi_process (mpi_communicator));
    CTy_matrix.reinit (mpi_communicator, dsp,
		       dof_handler_LS.n_locally_owned_dofs_per_processor (),
		       dof_handler_LS.n_locally_owned_dofs_per_processor (),
		       Utilities::MPI::this_mpi_process (mpi_communicator));
    if (dim == 3)
      {
	Cz_matrix.reinit (mpi_communicator, dsp,
			  dof_handler_LS.n_locally_owned_dofs_per_processor (),
			  dof_handler_LS.n_locally_owned_dofs_per_processor (),
			  Utilities::MPI::this_mpi_process (mpi_communicator));
	CTz_matrix.reinit (mpi_communicator, dsp,
			   dof_handler_LS.n_locally_owned_dofs_per_processor (),
			   dof_handler_LS.n_locally_owned_dofs_per_processor (),
			   Utilities::MPI::this_mpi_process (mpi_communicator));
      }
    dLij_matrix.reinit (mpi_communicator, dsp,
			dof_handler_LS.n_locally_owned_dofs_per_processor (),
			dof_handler_LS.n_locally_owned_dofs_per_processor (),
			Utilities::MPI::this_mpi_process (mpi_communicator));
    EntRes_matrix.reinit (mpi_communicator, dsp,
			  dof_handler_LS.n_locally_owned_dofs_per_processor (),
			  dof_handler_LS.n_locally_owned_dofs_per_processor (),
			  Utilities::MPI::this_mpi_process (mpi_communicator));
    SuppSize_matrix.reinit (
	mpi_communicator, dsp,
	dof_handler_LS.n_locally_owned_dofs_per_processor (),
	dof_handler_LS.n_locally_owned_dofs_per_processor (),
	Utilities::MPI::this_mpi_process (mpi_communicator));
    dCij_matrix.reinit (mpi_communicator, dsp,
			dof_handler_LS.n_locally_owned_dofs_per_processor (),
			dof_handler_LS.n_locally_owned_dofs_per_processor (),
			Utilities::MPI::this_mpi_process (mpi_communicator));
    A_matrix.reinit (mpi_communicator, dsp,
		     dof_handler_LS.n_locally_owned_dofs_per_processor (),
		     dof_handler_LS.n_locally_owned_dofs_per_processor (),
		     Utilities::MPI::this_mpi_process (mpi_communicator));
    LxA_matrix.reinit (mpi_communicator, dsp,
		       dof_handler_LS.n_locally_owned_dofs_per_processor (),
		       dof_handler_LS.n_locally_owned_dofs_per_processor (),
		       Utilities::MPI::this_mpi_process (mpi_communicator));
    Akp1_matrix.reinit (mpi_communicator, dsp,
			dof_handler_LS.n_locally_owned_dofs_per_processor (),
			dof_handler_LS.n_locally_owned_dofs_per_processor (),
			Utilities::MPI::this_mpi_process (mpi_communicator));
    LxAkp1_matrix.reinit (mpi_communicator, dsp,
			  dof_handler_LS.n_locally_owned_dofs_per_processor (),
			  dof_handler_LS.n_locally_owned_dofs_per_processor (),
			  Utilities::MPI::this_mpi_process (mpi_communicator));
...
template<int dim>
  ...
  LevelSetSolver<dim>::assemble_ML ()
  {
	...
	dof_handler_LS.begin_active (), endc_LS = dof_handler_LS.end ();
      ...
      if (cell_LS->is_locally_owned ())
	{
	  ...
	  constraints.distribute_local_to_global (cell_ML, local_dof_indices,
						  ML_vector);
	}
    // compress
...
template<int dim>
  ...
  LevelSetSolver<dim>::assemble_MC ()
  {
      ...
      if (cell_LS->is_locally_owned ())
	{
	  ...
	  // distribute
	  cell_LS->get_dof_indices (local_dof_indices);
	  constraints.distribute_local_to_global (cell_MC, local_dof_indices,
						  MC_matrix);
	}
    // compress
    MC_matrix.compress (VectorOperation::add);
    MC_preconditioner.reset (
      ...
      if (cell_LS->is_locally_owned ())
	{
	  ...
	  for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
		...
		for (unsigned int j = 0; j < dofs_per_cell_LS; j++)
		  {
		    cell_Cij_x (i, j) += (shape_grads_LS[j][0])
			* shape_values_LS[i] * JxW;
		    cell_Cij_y (i, j) += (shape_grads_LS[j][1])
			* shape_values_LS[i] * JxW;
		    cell_Cji_x (i, j) += (shape_grads_LS[i][0])
			* shape_values_LS[j] * JxW;
		    cell_Cji_y (i, j) += (shape_grads_LS[i][1])
			* shape_values_LS[j] * JxW;
		    if (dim == 3)
		      {
			cell_Cij_z (i, j) += (shape_grads_LS[j][2])
			    * shape_values_LS[i] * JxW;
			cell_Cji_z (i, j) += (shape_grads_LS[i][2])
			    * shape_values_LS[j] * JxW;
		      }
		  }
	    }
...
template<int dim>
      ...
      PETScWrappers::MPI::Vector &solution)
  {
	...
	get_vector_values (locally_relevant_solution_vx, gj_indices,
			   map_from_Q1_to_Q2, vx);
	get_vector_values (locally_relevant_solution_vy, gj_indices,
			   map_from_Q1_to_Q2, vy);
	if (dim == 3)
	  get_vector_values (locally_relevant_solution_vz, gj_indices,
			     map_from_Q1_to_Q2, vz);
	...
	for (int j = 0; j < ncolumns; j++)
	  {
		...
		double Compij = cK
		    * std::max (1 - std::pow (0.5 * (solni + soln[j]), 2), 0.0)
...
template<int dim>
  ...
  LevelSetSolver<dim>::assemble_EntRes_Matrix ()
  {
    ...
    entropy_mass = Utilities::MPI::sum (entropy_mass, mpi_communicator)
	/ volume;
    min_entropy = Utilities::MPI::min (min_entropy, mpi_communicator);
    max_entropy = Utilities::MPI::max (max_entropy, mpi_communicator);
    entropy_normalization_factor = std::max (
	std::abs (max_entropy - entropy_mass),
...
template<int dim>
      ...
      PETScWrappers::MPI::Vector &solution)
  {
	...
	for (int j = 0; j < ncolumns; j++)
	  LxAi[j] = Ai[j]
	      * ((Ai[j] > 0) ?
		  std::min (Rposi, Rneg[j]) : std::min (Rnegi, Rpos[j]));
...
template<int dim>
  ...
  {
    MPP_uH_solution = 0;
    compute_MPP_uH (MPP_uH_solution, MPP_uL_solution_ghosted,
		    NMPP_uH_solution_ghosted, un_solution);
	...
	for (int iter = 0; iter < NUM_ITER; iter++)
	  {
	    ...
	    // loop on locally owned i-DOFs (rows)
	    IndexSet::ElementIterator idofs_iter =
		locally_owned_dofs_LS.begin ();
	    for (; idofs_iter != locally_owned_dofs_LS.end (); idofs_iter++)
	      {
		int gi = *idofs_iter;
...
template<int dim>
      ...
      std::vector<PetscScalar> &values)
  {
	...
	if (index >= begin && index < end)
	  ...
	  {
	    const unsigned int ghostidx = ghost_indices.index_within_set (
		index);
