...
template<int dim>
  ...
  public:
    ...
    NavierStokesSolver (
	const unsigned int degree_LS, const unsigned int degree_U,
	const double time_step, const double eps, const double rho_air,
	const double nu_air, const double rho_fluid, const double nu_fluid,
	Function<dim> &force_function, const bool verbose,
	parallel::distributed::Triangulation<dim> &triangulation,
	MPI_Comm &mpi_communicator);
    // constructor for NOT LEVEL SET
    NavierStokesSolver (
	const unsigned int degree_LS, const unsigned int degree_U,
	const double time_step, Function<dim> &force_function,
	Function<dim> &rho_function, Function<dim> &nu_function,
	const bool verbose,
	parallel::distributed::Triangulation<dim> &triangulation,
	MPI_Comm &mpi_communicator);
    ...
    //boundary conditions
    void
    set_boundary_conditions (std::vector<unsigned int> boundary_values_id_u,
    ...
    void
    set_velocity (PETScWrappers::MPI::Vector locally_relevant_solution_u,
		  PETScWrappers::MPI::Vector locally_relevant_solution_v);
    void
    set_velocity (PETScWrappers::MPI::Vector locally_relevant_solution_u,
		  PETScWrappers::MPI::Vector locally_relevant_solution_v,
		  PETScWrappers::MPI::Vector locally_relevant_solution_w);
    void
    set_phi (PETScWrappers::MPI::Vector locally_relevant_solution_phi);
    void
    get_pressure (PETScWrappers::MPI::Vector &locally_relevant_solution_p);
    void
    get_velocity (PETScWrappers::MPI::Vector &locally_relevant_solution_u,
		  PETScWrappers::MPI::Vector &locally_relevant_solution_v);
    void
    get_velocity (PETScWrappers::MPI::Vector &locally_relevant_solution_u,
		  PETScWrappers::MPI::Vector &locally_relevant_solution_v,
...
template<int dim>
  NavierStokesSolver<dim>::NavierStokesSolver (
      ...
      parallel::distributed::Triangulation<dim> &triangulation,
      MPI_Comm &mpi_communicator) :
      mpi_communicator (mpi_communicator), triangulation (triangulation), degree_LS (
	  ...
	  triangulation), fe_P (degree_U - 1), force_function (force_function),
      //This is dummy since rho and nu functions won't be used
      rho_function (force_function), nu_function (force_function), rho_air (
	  rho_air), nu_air (nu_air), rho_fluid (rho_fluid), nu_fluid (nu_fluid), time_step (
	  time_step), eps (eps), verbose (verbose), LEVEL_SET (1), RHO_TIMES_RHS (
	  1), pcout (
	  std::cout,
	  (Utilities::MPI::this_mpi_process (mpi_communicator) == 0)), rebuild_Matrix_U (
	  true), rebuild_S_M (true), rebuild_Matrix_U_preconditioners (true), rebuild_S_M_preconditioners (
	  true)
  {
    setup ();
  }
...
template<int dim>
  NavierStokesSolver<dim>::NavierStokesSolver (
	  ...
	  degree_LS), dof_handler_LS (triangulation), fe_LS (degree_LS), degree_U (
	  degree_U), dof_handler_U (triangulation), fe_U (degree_U), dof_handler_P (
	  triangulation), fe_P (degree_U - 1), force_function (force_function), rho_function (
	  rho_function), nu_function (nu_function), time_step (time_step), verbose (
	  verbose), LEVEL_SET (0), RHO_TIMES_RHS (0), pcout (
	  std::cout,
	  (Utilities::MPI::this_mpi_process (mpi_communicator) == 0)), rebuild_Matrix_U (
	  true), rebuild_S_M (true), rebuild_Matrix_U_preconditioners (true), rebuild_S_M_preconditioners (
	  true)
...
template<int dim>
  void
  NavierStokesSolver<dim>::set_rho_and_nu_functions (
      const Function<dim> &rho_function, const Function<dim> &nu_function)
  {
...
template<int dim>
  void
  NavierStokesSolver<dim>::initial_condition (
      PETScWrappers::MPI::Vector locally_relevant_solution_phi,
      PETScWrappers::MPI::Vector locally_relevant_solution_u,
      PETScWrappers::MPI::Vector locally_relevant_solution_v,
      PETScWrappers::MPI::Vector locally_relevant_solution_p)
...
template<int dim>
  void
  NavierStokesSolver<dim>::set_boundary_conditions (
      std::vector<unsigned int> boundary_values_id_u,
      std::vector<unsigned int> boundary_values_id_v,
      std::vector<double> boundary_values_u,
      std::vector<double> boundary_values_v)
...
template<int dim>
  void
  NavierStokesSolver<dim>::set_velocity (
      PETScWrappers::MPI::Vector locally_relevant_solution_u,
      PETScWrappers::MPI::Vector locally_relevant_solution_v)
  {
    this->locally_relevant_solution_u = locally_relevant_solution_u;
...
template<int dim>
  void
  NavierStokesSolver<dim>::set_phi (
      PETScWrappers::MPI::Vector locally_relevant_solution_phi)
  {
    this->locally_relevant_solution_phi = locally_relevant_solution_phi;
  }
...
template<int dim>
  void
  NavierStokesSolver<dim>::get_rho_and_nu (double phi)
  {
    double H = 0;
    // get rho, nu
    if (phi > eps)
...
template<int dim>
  void
  NavierStokesSolver<dim>::get_pressure (
      PETScWrappers::MPI::Vector &locally_relevant_solution_p)
  {
...
template<int dim>
  void
  NavierStokesSolver<dim>::get_velocity (
      PETScWrappers::MPI::Vector &locally_relevant_solution_u,
      PETScWrappers::MPI::Vector &locally_relevant_solution_v)
  {
...
template<int dim>
  void
  NavierStokesSolver<dim>::setup_DOF ()
  {
    rho_min = 1.;
...
template<int dim>
  void
  NavierStokesSolver<dim>::setup_VECTORS ()
  {
    ...
    locally_relevant_solution_u_old.reinit (locally_owned_dofs_U,
					    locally_relevant_dofs_U,
					    mpi_communicator);
    ...
    locally_relevant_solution_v_old.reinit (locally_owned_dofs_U,
					    locally_relevant_dofs_U,
					    mpi_communicator);
    ...
    locally_relevant_solution_w_old.reinit (locally_owned_dofs_U,
					    locally_relevant_dofs_U,
					    mpi_communicator);
    locally_relevant_solution_w_old = 0;
    //init vectors for dpsi
    locally_relevant_solution_psi.reinit (locally_owned_dofs_P,
					  locally_relevant_dofs_P,
					  mpi_communicator);
    locally_relevant_solution_psi = 0;
    system_rhs_psi.reinit (locally_owned_dofs_P, mpi_communicator);
    //init vectors for dpsi old
    locally_relevant_solution_psi_old.reinit (locally_owned_dofs_P,
					      locally_relevant_dofs_P,
					      mpi_communicator);
    locally_relevant_solution_psi_old = 0;
    //init vectors for q
    completely_distributed_solution_q.reinit (locally_owned_dofs_P,
					      mpi_communicator);
    system_rhs_q.reinit (locally_owned_dofs_P, mpi_communicator);
    //init vectors for psi
    completely_distributed_solution_psi.reinit (locally_owned_dofs_P,
						mpi_communicator);
    //init vectors for p
    ...
    DoFTools::make_sparsity_pattern (dof_handler_U, dsp_Matrix, constraints,
				     false);
    SparsityTools::distribute_sparsity_pattern (
	dsp_Matrix, dof_handler_U.n_locally_owned_dofs_per_processor (),
	mpi_communicator, locally_relevant_dofs_U);
    system_Matrix_u.reinit (
	mpi_communicator, dsp_Matrix,
	dof_handler_U.n_locally_owned_dofs_per_processor (),
	dof_handler_U.n_locally_owned_dofs_per_processor (),
	Utilities::MPI::this_mpi_process (mpi_communicator));
    system_Matrix_v.reinit (
	mpi_communicator, dsp_Matrix,
	dof_handler_U.n_locally_owned_dofs_per_processor (),
	dof_handler_U.n_locally_owned_dofs_per_processor (),
	Utilities::MPI::this_mpi_process (mpi_communicator));
    system_Matrix_w.reinit (
	mpi_communicator, dsp_Matrix,
	dof_handler_U.n_locally_owned_dofs_per_processor (),
	dof_handler_U.n_locally_owned_dofs_per_processor (),
	Utilities::MPI::this_mpi_process (mpi_communicator));
    ...
    DoFTools::make_sparsity_pattern (dof_handler_P, dsp_S, constraints_psi,
				     false);
    SparsityTools::distribute_sparsity_pattern (
	dsp_S, dof_handler_P.n_locally_owned_dofs_per_processor (),
    ...
    DoFTools::make_sparsity_pattern (dof_handler_P, dsp_M, constraints_psi,
				     false);
    SparsityTools::distribute_sparsity_pattern (
	dsp_M, dof_handler_P.n_locally_owned_dofs_per_processor (),
	mpi_communicator, locally_relevant_dofs_P);
    system_M.reinit (mpi_communicator, dsp_M,
		     dof_handler_P.n_locally_owned_dofs_per_processor (),
		     dof_handler_P.n_locally_owned_dofs_per_processor (),
		     Utilities::MPI::this_mpi_process (mpi_communicator));
    rebuild_S_M = true;
...
template<int dim>
  void
  NavierStokesSolver<dim>::init_constraints ()
  {
    //grl constraints
    constraints.clear ();
...
template<int dim>
  void
  NavierStokesSolver<dim>::assemble_system_U ()
  {
    ...
    typename DoFHandler<dim>::active_cell_iterator cell_LS =
      ...
      if (cell_U->is_locally_owned ())
	{
	  ...
	  fe_values_LS.get_function_values (locally_relevant_solution_phi,
					    phiqnp1);
	  // get function values for U
	  fe_values_U.get_function_values (locally_relevant_solution_u, uqn);
	  fe_values_U.get_function_values (locally_relevant_solution_u_old,
					   uqnm1);
	  fe_values_U.get_function_values (locally_relevant_solution_v, vqn);
	  fe_values_U.get_function_values (locally_relevant_solution_v_old,
					   vqnm1);
	  if (dim == 3)
	    {
	      fe_values_U.get_function_values (locally_relevant_solution_w,
					       wqn);
	      fe_values_U.get_function_values (locally_relevant_solution_w_old,
					       wqnm1);
	    }
	  // For explicit nonlinearity
	  ...
	  fe_values_P.get_function_gradients (locally_relevant_solution_p,
					      grad_pqn);
	  fe_values_P.get_function_gradients (locally_relevant_solution_psi,
					      grad_psiqn);
	  fe_values_P.get_function_gradients (locally_relevant_solution_psi_old,
					      grad_psiqnm1);
	  ...
	  for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
	      ...
	      for (unsigned int i = 0; i < dofs_per_cell; ++i)
		{
	      ...
	      pressure_grad_u = (grad_pqn[q_point][0]
		  + 4. / 3 * grad_psiqn[q_point][0]
		  - 1. / 3 * grad_psiqnm1[q_point][0]);
	      pressure_grad_v = (grad_pqn[q_point][1]
		  + 4. / 3 * grad_psiqn[q_point][1]
		  - 1. / 3 * grad_psiqnm1[q_point][1]);
	      if (dim == 3)
		pressure_grad_w = (grad_pqn[q_point][2]
		    + 4. / 3 * grad_psiqn[q_point][2]
		    - 1. / 3 * grad_psiqnm1[q_point][2]);
		...
		{
		  rho_value = rho_function.value (
		      fe_values_U.quadrature_point (q_point));
		  nu_value = nu_function.value (
		      fe_values_U.quadrature_point (q_point));
		}
		...
		w_star = 2 * wqn[q_point] - wqnm1[q_point];
	      ...
	      // FORCE TERMS
	      force_function.vector_value (
		  fe_values_U.quadrature_point (q_point), force_terms);
	      ...
	      if (RHO_TIMES_RHS == 1)
		{
	      ...
	      for (unsigned int i = 0; i < dofs_per_cell; ++i)
		{
		  cell_rhs_u (i) += ((4. / 3 * rho * uqn[q_point]
		      - 1. / 3 * rho * uqnm1[q_point]
		      + 2. / 3 * time_step * (force_u - pressure_grad_u)
		  //-2./3*time_step*rho*nonlinearity_u
		  ) * shape_value[i]) * JxW;
		  cell_rhs_v (i) += ((4. / 3 * rho * vqn[q_point]
		      - 1. / 3 * rho * vqnm1[q_point]
		      + 2. / 3 * time_step * (force_v - pressure_grad_v)
		  //-2./3*time_step*rho*nonlinearity_v
		  ) * shape_value[i]) * JxW;
		  if (dim == 3)
		    cell_rhs_w (i) += ((4. / 3 * rho * wqn[q_point]
			- 1. / 3 * rho * wqnm1[q_point]
		  ...
		  if (rebuild_Matrix_U == true)
		    for (unsigned int j = 0; j < dofs_per_cell; ++j)
		      {
			if (dim == 2)
			  cell_A_u (i, j) += (rho_star * shape_value[i]
			      * shape_value[j]
			      + 2. / 3 * time_step * nu_value
				  * (shape_grad[i] * shape_grad[j])
			      + 2. / 3 * time_step * rho * shape_value[i]
				  * (u_star * shape_grad[j][0]
				      + v_star * shape_grad[j][1]) // semi-implicit NL
			  ) * JxW;
			else
			  //dim==3
			  cell_A_u (i, j) += (rho_star * shape_value[i]
				  ...
				  * (shape_grad[i] * shape_grad[j])
			      + 2. / 3 * time_step * rho * shape_value[i]
				  * (u_star * shape_grad[j][0]
				      + v_star * shape_grad[j][1]
				      + w_star * shape_grad[j][2]) // semi-implicit NL
			  ) * JxW;
		      }
		}
	    }
	  ...
	  // distribute
	  if (rebuild_Matrix_U == true)
	    constraints.distribute_local_to_global (cell_A_u, local_dof_indices,
						    system_Matrix_u);
	  constraints.distribute_local_to_global (cell_rhs_u, local_dof_indices,
						  system_rhs_u);
	  constraints.distribute_local_to_global (cell_rhs_v, local_dof_indices,
						  system_rhs_v);
	  if (dim == 3)
	    constraints.distribute_local_to_global (cell_rhs_w,
						    local_dof_indices,
						    system_rhs_w);
	}
    ...
    if (dim == 3)
      system_rhs_w.compress (VectorOperation::add);
    if (rebuild_Matrix_U == true)
      {
...
template<int dim>
  void
  NavierStokesSolver<dim>::assemble_system_dpsi_q ()
  {
    if (rebuild_S_M == true)
      {
	system_S = 0;
      ...
      if (cell_P->is_locally_owned ())
	{
	  ...
	  fe_values_U.get_function_gradients (locally_relevant_solution_u,
					      gunp1);
	  fe_values_U.get_function_gradients (locally_relevant_solution_v,
					      gvnp1);
	  if (dim == 3)
	    fe_values_U.get_function_gradients (locally_relevant_solution_w,
						gwnp1);
	  ...
	  for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
		...
		// rho and nu are defined through functions
		nu_value = nu_function.value (
		    fe_values_U.quadrature_point (q_point));

	      for (unsigned int i = 0; i < dofs_per_cell; ++i)
		{
		  cell_rhs_psi (i) += -3. / 2. / time_step * rho_min * divU
		      * shape_value[i] * JxW;
		  ...
		  if (rebuild_S_M == true)
		      ...
		      if (i == j)
			{
			  cell_S (i, j) += shape_grad[i] * shape_grad[j] * JxW
			      + 1E-10;
			  cell_M (i, j) += shape_value[i] * shape_value[j]
			      * JxW;
			}
		      else
			{
	  ...
	  if (rebuild_S_M == true)
	    {
	      constraints_psi.distribute_local_to_global (cell_S,
							  local_dof_indices,
							  system_S);
	      constraints_psi.distribute_local_to_global (cell_M,
							  local_dof_indices,
							  system_M);
	    }
	  constraints_psi.distribute_local_to_global (cell_rhs_q,
						      local_dof_indices,
						      system_rhs_q);
	  constraints_psi.distribute_local_to_global (cell_rhs_psi,
						      local_dof_indices,
						      system_rhs_psi);
	}
    if (rebuild_S_M == true)
	...
	if (rebuild_S_M_preconditioners)
	  {
	    ...
	    preconditioner_S.reset (
		new PETScWrappers::PreconditionBoomerAMG (
		    system_S,
		    PETScWrappers::PreconditionBoomerAMG::AdditionalData (
			true)));
	    preconditioner_M.reset (
		new PETScWrappers::PreconditionBoomerAMG (
		    system_M,
		    PETScWrappers::PreconditionBoomerAMG::AdditionalData (
			true)));
 ...
 */
	...
	int r = row;
	//printf("Now is trying to set :value[%d]=%f form process %d  \n",r,value,P);
...
template<int dim>
      ...
      const PETScWrappers::MPI::Vector &rhs)
  {
    ...
    constraints.distribute (completely_distributed_solution);
    solver.solve (Matrix, completely_distributed_solution, rhs,
		  *preconditioner);
    constraints.distribute (completely_distributed_solution);
    if (solver_control.last_step () > MAX_NUM_ITER_TO_RECOMPUTE_PRECONDITIONER)
...
template<int dim>
  ...
  NavierStokesSolver<dim>::get_velocity ()
  {
    ...
    if (dim == 3)
      {
	solve_U (constraints, system_Matrix_w, preconditioner_Matrix_w,
		 completely_distributed_solution_w, system_rhs_w);
...
template<int dim>
  ...
  NavierStokesSolver<dim>::get_pressure ()
  {
    // GET DPSI
    assemble_system_dpsi_q ();
    solve_P (constraints_psi, system_S, preconditioner_S,
	     completely_distributed_solution_psi, system_rhs_psi);
    locally_relevant_solution_psi = completely_distributed_solution_psi;
    // SOLVE Q
    solve_P (constraints, system_M, preconditioner_M,
	     completely_distributed_solution_q, system_rhs_q);
    // UPDATE THE PRESSURE
    completely_distributed_solution_p.add (1,
					   completely_distributed_solution_psi);
    completely_distributed_solution_p.add (1,
					   completely_distributed_solution_q);
...
template<int dim>
  void
  NavierStokesSolver<dim>::nth_time_step ()
  {
    get_velocity ();
    get_pressure ();
  }
...
template<int dim>
  void
  NavierStokesSolver<dim>::save_old_solution ()
  {
